...
function wprls_deentitize_str( str ) {
    ...
    jQuery( document ).ready(function( $ ) {
        ...
        $('.wprlslayerslider').each(function() {
            ...
            if ( ! options.sliderskin )
                selector.addClass('wprls_skin_default');
            ...
            if ( options.pause_on_mouse_over )
                on_hover = 'pause';
            else
                on_hover = 'none';

            if ( options.slide_order == 'seq'  )
                seq_or_random = false;
            else if ( options.slide_order == 'rnd' )
                seq_or_random = true;

            if ( options.nav_skin === '1' ) { nav_arrows = true; nav_buttons = false; }

            if ( options.nav_skin === '2' ) { nav_arrows = false; nav_buttons = false }

            if ( options.nav_skin === '3' ) { nav_arrows = true; nav_buttons = true }

            if ( options.nav_skin === '4' ) { nav_arrows = false; nav_buttons = true }
            ...
            $(selector).sliderPro({
                ...
                responsive: options.is_responsive,
                width: width,
                height: height,
                autoplay: options.auto_start,
                autoplayOnHover: on_hover,
                shuffle: seq_or_random,
                arrows: nav_arrows,
                buttons: nav_buttons,
                fade: true,
                fadeDuration: 700,
                autoplayDelay: options.autoplay_delay,
                init: function() {

                   layer = this.$slider.find('.sp-slide').first().find('.sp-layer')
                   trans_duration = this.$slider.find('.sp-slide').first().data('transduration');
                ...
                gotoSlide: function( event ) {
                    ...
                    prev_index = event.previousIndex;

                    cur_layer = this.$slider.find('.sp-slide'+cur_index).find('.sp-layer');
                    prev_layer = this.$slider.find('.sp-slide'+prev_index).find('.sp-layer');

                    slide_duration = this.$slider.find('.sp-slide'+cur_index).data('slideduration');

                    trans_duration = this.$slider.find('.sp-slide'+cur_index).data('transduration');

                    

                    prev_layer.each(function( index ){
                        $(this).removeClass('animated');
                        $(this).removeClass($(this).data('animation')).hide();
                    });


                    cur_layer.each( function( index ) {
                   ...
                   //  setTimeout(function(){
                   //     cur_layer.addClass('animated fadeIn');
                   //  }, trans_duration);
